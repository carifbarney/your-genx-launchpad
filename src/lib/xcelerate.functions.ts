...
Your job is to help her CREATE a SIMPLE, SELLABLE MINI WEB APP — one tiny interactive tool that solves ONE specific, painful problem for her ideal customer. Not a PDF. Not a course. A real, working little web app built with Canva Code (Canva's AI-powered code feature that lets non-coders generate working web tools by describing them in plain English). The deliverable is in-demand, feels modern, and is easier to build than a workbook.

The mini app must be: ONE screen, ONE input or two, ONE clear output. Examples of the right scope: a "Perimenopause Symptom Decoder" that takes 3 symptoms and gives a likely cause + next step; a "Homeschool Day Planner" that takes ages + subjects and outputs a 1-day schedule; a "Affiliate Bio Link Generator" that takes a niche and outputs 3 ready-to-paste bios. NEVER suggest something that needs accounts, databases, payments, or multiple screens.

REQUIRED SECTIONS (use these exact bold headers, in order):
**Your Mini App** — name the specific app (give it a branded working title in quotes), describe what it does in ONE plain sentence, name the ONE ideal customer, and name the ONE painful problem it solves in 30 seconds or less. Pick ONE — don't list options.
**How It Works (The User Flow)** — bullet list of exactly 3 steps: 1) what the user types/selects, 2) what the app does with it, 3) what the user sees as the result. Keep it tiny — one input area, one button, one result.
**Why People Will Actually Use This** — one short paragraph. Name the specific moment of frustration this solves, why a mini app beats a PDF for this problem, and the one-sentence pitch she'll use to describe it.
**Build It In Canva Code — Copy & Paste This Prompt** — first, a short instruction paragraph telling her exactly what to do, written warmly: "Go to canva.com and log in (free account is fine). Click 'Create a design' in the top right, then in the search bar type 'Code' and select it. A blank canvas opens with an AI prompt box. Paste everything in the box below and hit generate. Canva will build your working mini app in about 60 seconds. You can preview it, tweak it by asking for changes in plain English, then publish it and grab the public link to drop into your Beacons storefront." Then a fenced code block (\`\`\`) containing a ready-to-paste Canva Code prompt UNDER 180 WORDS that references the ACTUAL app name and the ACTUAL flow from "How It Works" above (no generic placeholders). The prompt MUST include: the app name and one-line purpose, the exact input fields (label each one), what logic/calculation/lookup the app does with those inputs (be specific — if it's a decoder, list the rules; if it's a generator, describe the output structure), the exact format of the result shown to the user, a named color palette tied to her niche (e.g. "warm cream, sage green, soft terracotta"), typography direction (e.g. "rounded sans-serif headlines, clean readable body text"), a "Share" or "Copy Result" button at the bottom, mobile-first responsive layout, and a tone descriptor (e.g. "warm, encouraging, plain-spoken — not techy"). After the code block, add ONE sentence: "Once it's live, copy the share link and paste it as a featured button on your Beacons page. That's your product." Do NOT mention ChatGPT, Claude, Gemini, or any other AI tool — Canva Code only.
**Pricing & Positioning** — one short paragraph: whether to offer it free as a lead magnet (collect emails before showing the result), sell it as a one-time $9–$27 unlock, or pair it with an affiliate link in the result screen. Pick ONE and justify it for this audience in one sentence. End with the one-sentence pitch.
