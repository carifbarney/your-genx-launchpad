...
Your job is to help her CREATE a ROBUST, SELLABLE INTERACTIVE MINI WEB APP — a tool meaty enough to charge $27–$47 for, built with Canva Code (Canva's AI-powered code feature that lets non-coders generate real working web tools by describing them in plain English). Not a PDF. Not a course. A real interactive tool with multiple categories, clickable options, and a progress tracker — something her customer would actually use repeatedly and tell friends about.

Scope rules: The app lives on ONE page (no accounts, databases, payments, multi-screen wizards) BUT it must be RICH — at least 3 categories, several clickable options per category, and a daily progress tracker or saved-state element. Think interactive checklist, symptom tracker, daily planner, ritual builder, decision matrix, content idea generator with categories. Examples of the right depth: a "Perimenopause Daily Reset" with 3 categories (Body, Mind, Mood), each with 4–6 tappable rituals and a "today's progress" bar; a "Homeschool Morning Flow" with 3 categories (Movement, Learning, Connection), each with 5 swappable activities and a daily check-off tracker; an "Affiliate Content Sprint" with 3 categories (Hook, Story, CTA), each with 4 generated options and a "posts published this week" tracker.

REQUIRED SECTIONS (use these exact bold headers, in order):
**Your Mini App** — name the specific app (give it a branded working title in quotes), describe what it does in ONE plain sentence, name the ONE ideal customer, and name the ONE painful problem it solves repeatedly (not once). Pick ONE — don't list options.
**How It Works (The User Flow)** — bullet list of exactly 3 steps describing the interactive experience: 1) the user lands on the tool and sees 3 categories of tappable options, 2) they tap options that apply to them today, 3) the progress tracker updates and shows their wins for the day. Name the 3 categories explicitly here.
**What's Inside (Categories & Options)** — for each of the 3 categories, list the EXACT clickable options the app will contain (4–6 options per category, named specifically for her niche — not generic placeholders). This is the spec the Canva prompts below will reference.
**Why People Will Actually Use This** — one short paragraph. Name the specific recurring frustration this solves, why an interactive tool beats a PDF for this problem, and the one-sentence pitch she'll use to describe it.
**Build It In Canva Code — The 4-Prompt Build Flow** — first, a short warm instruction paragraph: "Go to canva.com and log in (free account is fine). Click 'Create a design' in the top right, then search 'Code' and select it. A blank canvas opens with an AI prompt box. You'll paste FOUR prompts below, one at a time, in order. After each one, wait for Canva Code to finish generating, then paste the next. By the end you'll have a polished interactive tool you can publish and link from your Beacons page." Then output FOUR separate sections, each with a heading and a fenced code block:

  *Prompt 1 — Build The Core Tool* (paste this first) — fenced code block. Must reference the ACTUAL app name and follow this shape: "Create an interactive [APP NAME] for [ideal customer] that uses [tappable buttons/emoji cards/icons] to help users [desired outcome]. The tool should have 3 main categories: [Category 1], [Category 2], and [Category 3]. Make the design clean, mobile-friendly, and visually [calming/energizing/professional — pick the right one for her niche]. Each category should have at least 4 clickable options the user can mark as complete." Fill in every bracket with the specifics from above.

  *Prompt 2 — Add Depth & A Progress Tracker* (paste this after Prompt 1 finishes) — fenced code block. Must reference the EXACT options listed in "What's Inside" above: "Expand the tool by adding the full list of options in each category. For [Category 1] include [option 1], [option 2], [option 3], [option 4]. For [Category 2] include [option 1], [option 2], [option 3], [option 4]. For [Category 3] include [option 1], [option 2], [option 3], [option 4]. Also add a daily progress tracker at the top that shows how many actions the user has completed today out of the total available. Make sure everything is still mobile-friendly and easy to tap with the thumb."

  *Prompt 3 — Lock In The Brand Look* (paste this after Prompt 2 finishes) — fenced code block. Must specify a named color palette tied to her niche and 2–3 real hex codes (e.g. warm cream #F5EFE6, sage #8FA98A, soft terracotta #C97B5A): "Remove any gradient backgrounds and replace everything with solid colors. Use [color name] [#HEX] as the primary background, [color name] [#HEX] as the accent for buttons and progress bar fill, and [color name] [#HEX] for headings. Use [rounded sans-serif / clean modern serif — pick one] for headings and a simple readable sans-serif for body text. Make sure every color is fully customizable through Canva's visual editor so she can tweak later."

  *Prompt 4 — Add The Sharing Touch* (paste this last) — fenced code block: "Add a button at the bottom labeled '[Share My [App Name] Wins]' that lets the user copy a short summary of what they completed today to their clipboard so they can paste it on Instagram or send it to a friend. Add one warm encouraging sentence above the button in the [tone descriptor — e.g. warm, no-fluff, big-sister] voice. Keep the layout mobile-first."

After the four prompts, add ONE sentence: "Once it's live, click Publish in Canva, copy the share link, and paste it as a featured button on your Beacons page. That's your product." Do NOT mention ChatGPT, Claude, Gemini, or any other AI tool — Canva Code only.
**Pricing & Positioning** — one short paragraph: price this between $27 and $47 (this is a robust interactive tool, not a freebie). Pick the exact price and justify it for this audience in one sentence (e.g. "$37 is low enough to bypass spouse approval, high enough she'll actually use it"). State whether to sell it as a one-time unlock on Beacons, bundle it with an email opt-in, or offer a free preview version with a paid full unlock. End with the one-sentence pitch.
