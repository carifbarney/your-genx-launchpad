...
const VinylRecord = () => (
  <svg
    ...
    style={{
      ...
      filter: "drop-shadow(0 0 6px #00F0D1) drop-shadow(0 0 12px rgba(0,240,209,0.7))",
    }}
  >
...
function Dashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      Promise.all([
        fetchPlan({})
          .then((r) => setPlan(r.plan as PlanRow))
          .catch(() => {}),
        fetchRemaining({})
    ...
    <main className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute inset-x-0 bottom-0 top-1/2">
          <div className="xcel-grid-floor" />
        </div>
        <div className="absolute -top-32 -left-24 h-[420px] w-[420px] rounded-full bg-[#FE2DA3] opacity-25 blur-3xl xcel-blob" />
        <div
          className="absolute top-40 -right-32 h-[480px] w-[480px] rounded-full bg-[#8A2BE2] opacity-25 blur-3xl xcel-blob"
          style={{ animationDelay: "-6s" }}
        />
        <div
          className="absolute bottom-0 left-1/3 h-[380px] w-[380px] rounded-full bg-[#00F0D1] opacity-15 blur-3xl xcel-blob"
      ...
      <header className="relative border-b border-[#FE2DA3]/20 bg-black/40 backdrop-blur-md">
        <span
          ...
          style={{ background: "linear-gradient(90deg, transparent, #FE2DA3 30%, #00F0D1 70%, transparent)" }}
        />
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-3">
          <img
            src={xcelerateLogo}
            alt="Xcelerate"
            className="h-10 w-auto drop-shadow-[0_0_12px_rgba(254,45,163,0.55)]"
          />
          <div className="flex items-center gap-4">
            <span className="hidden font-mono text-[11px] uppercase tracking-widest text-[#00F0D1]/80 sm:inline">
              {email}
            </span>
      ...
      <section className="relative mx-auto max-w-5xl px-6 py-10">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4 xcel-fade-up">
            ...
            <div className="mt-4 flex w-fit items-center gap-2 rounded-full border border-[#FE2DA3]/40 bg-black/60 px-3 py-1 backdrop-blur">
              ...
              <span className="xcel-kicker text-[#F5F2EC]">
                {completedCount === 0
                  ? "Side A — Track 1"
                  : completedCount === 4
                    ? "Album complete — go gold"
                    : `${completedCount} of 4 in the can`}
              </span>
            </div>
          ...
          {(plan?.niche || plan?.starting_point_output) && (
            <button onClick={handleStartFresh} className="xcel-btn-ghost rounded-full px-4 py-2 text-xs">
              ↺ Rewind tape
            </button>
          )}
        </div>
        ...
        <div className="mb-8 xcel-fade-up" style={{ animationDelay: "0.05s" }}>
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-mono text-xs font-bold uppercase tracking-widest text-[#00F0D1]">
          ...
          <div className="relative h-2.5 w-full overflow-hidden rounded-full border border-[#FE2DA3]/30 bg-black/60">
            <div
              ...
              style={{
                width: `${progressPct}%`,
                background: "linear-gradient(90deg, #FE2DA3, #8A2BE2, #00F0D1)",
                boxShadow: "0 0 16px rgba(254,45,163,0.7)",
              }}
            />
          </div>
        </div>
              ...
              >
                {active && (
                  <span
                    ...
                    style={{
                      background: "linear-gradient(135deg, rgba(254,45,163,0.18) 0%, rgba(138,43,226,0.18) 100%)",
                    }}
                  />
                )}
                {active && (
                  <span
                    ...
                    style={{ background: "linear-gradient(90deg, #FE2DA3, #00F0D1)", boxShadow: "0 0 10px #FE2DA3" }}
                  />
                )}
                <div className="flex items-center justify-between">
                  <span
                    className={`text-2xl leading-none transition-transform duration-300 ${active ? "scale-110" : "group-hover:scale-110"}`}
                  >
                    {t.emoji}
                  </span>
                  {done && (
                ...
                <div className="mt-3 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#FE2DA3]">
                  Track 0{t.num}
                </div>
                <div
                  className="mt-1 font-[var(--font-display)] text-lg leading-tight tracking-wide text-[#F5F2EC]"
...
function ToolPanel({
    ...
    () => [
      {
        ...
        required: true,
        placeholder:
          'e.g. "Building marketing strategies, managing teams, creating systems..." — or "Honestly, not sure yet"',
        chips: [
      ...
      },
      {
        ...
        value: theirFrustration,
        set: setTheirFrustration,
        required: true,
  ...
  const prerequisite = useMemo(() => {
    if (tool === "product")
      return plan?.starting_point_output ? null : "Finish Step 1 (Starting Point) first so we know your niche.";
    if (tool === "storefront")
      return plan?.product_output ? null : "Finish Step 2 (Product Builder) first so we know what you're selling.";
    if (tool === "launch_plan")
      return plan?.product_output ? null : "Finish Step 2 (Product Builder) first so we have something to launch.";
    return null;
  }, [tool, plan]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (tool === "starting_point") {
      ...
      if (!hoursPerDay.trim()) {
        setErrorMsg("Tell me roughly how much time you have each day.");
        return;
      }
  ...
  return (
    <div className="xcel-card xcel-scanlines relative overflow-hidden p-6 backdrop-blur-md sm:p-8 xcel-fade-up">
      <span
        aria-hidden
        className="absolute inset-x-0 top-0 h-[2px]"
        style={{ background: "linear-gradient(90deg, #FE2DA3, #8A2BE2, #00F0D1)", boxShadow: "0 0 12px #FE2DA3" }}
      />
      ...
      {prerequisite ? (
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          {tool === "starting_point" && (
            <>
              ...
              <div className="relative -mx-2 mb-2 rounded-2xl border border-[#FE2DA3]/40 bg-black/60 px-5 py-5 shadow-[0_0_24px_rgba(254,45,163,0.18)] sm:-mx-0">
                <p className="font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-[#00F0D1]/90">
                  Stage 01 — Expertise Excavation
                </p>
                <h3
                  ...
                  style={{
                    ...
                    color: "#FE2DA3",
                    textShadow:
                      "0 0 10px rgba(254,45,163,0.85), 0 0 22px rgba(254,45,163,0.55), 0 0 40px rgba(254,45,163,0.35)",
              ...
              <div className="flex items-center justify-between gap-3">
                <p className="font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-[#F5F2EC]/60">
                  Question {String(spStep + 1).padStart(2, "0")}{" "}
                  <span className="text-[#F5F2EC]/30">of {String(startingPointSteps.length).padStart(2, "0")}</span>
                </p>
              ...
              {!isLastStep && (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (canAdvance) setSpStep((s) => Math.min(startingPointSteps.length - 1, s + 1));
                      else setErrorMsg("Give this one a shot before moving on.");
                    }}
          ...
          {tool === "storefront" && (
            <>
              ...
              <Field
                ...
                value={storefrontNotes}
                onChange={setStorefrontNotes}
                disabled={streaming}
          ...
          {tool === "launch_plan" && (
            <>
              ...
              <Field
                ...
                onChange={setPlatformPreference}
                disabled={streaming}
          ...
          {errorMsg && (
            <p className="rounded-md border border-[#FF3B6B]/40 bg-[#FF3B6B]/10 px-4 py-3 text-sm font-semibold text-[#FF8FAA]">
              {errorMsg}
            </p>
          )}
          ...
          {disabled && !streaming && (
            <p className="rounded-xl border border-[#00F0D1]/40 bg-[#00F0D1]/10 px-4 py-3 text-sm font-semibold text-[#00F0D1]">
              That's a wrap — all 20 AI runs burned for today. Tape resets at midnight. Come back tomorrow and pick up
              where you left off.
            </p>
          )}
      ...
      {(output || streaming) && (
        <div
          ref={outputRef}
          className="relative mt-8 overflow-hidden rounded-2xl border border-[#00F0D1]/30 bg-black/60 p-6 shadow-[0_0_24px_rgba(0,240,209,0.10)] xcel-fade-up"
        >
          {streaming && !output && (
            <div className="flex flex-col items-start gap-4 py-6">
              <div className="flex items-center gap-2 text-base font-semibold text-[#F5F2EC]">
                <span className="inline-flex gap-1">
                  <span
                    className="h-2 w-2 rounded-full shadow-[0_0_8px_#FE2DA3]"
                    style={{
                      background: "#FE2DA3",
                      animation: "xcel-bounce-dot 1.4s infinite ease-in-out",
                      animationDelay: "0s",
                    }}
                  />
                  <span
                    className="h-2 w-2 rounded-full shadow-[0_0_8px_#8A2BE2]"
                    style={{
                      background: "#8A2BE2",
                      animation: "xcel-bounce-dot 1.4s infinite ease-in-out",
                  ...
                  />
                  <span
                    className="h-2 w-2 rounded-full shadow-[0_0_8px_#00F0D1]"
                    style={{
                      background: "#00F0D1",
                      animation: "xcel-bounce-dot 1.4s infinite ease-in-out",
            ...
            <>
              {streaming && (
                <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#FE2DA3]/50 bg-[#FE2DA3]/10 px-3 py-1 font-mono text-xs font-bold uppercase tracking-widest text-[#FE2DA3]">
                  <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#FE2DA3] shadow-[0_0_6px_#FE2DA3]" /> ●
                  REC — live
                </div>
              )}
              {tool === "starting_point" ? (
              ...
              {!streaming && tool !== "starting_point" && (
                <div className="mt-6 flex flex-wrap items-center gap-3">
                  <button
                    onClick={handleCopy}
                    className="xcel-btn-ghost inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs"
                  >
                    {copied ? "✓ Copied!" : "📋 Copy"}
                  </button>
                  {NEXT_TOOL[tool] && (
  ...
  return (
    <div className="relative z-10 flex items-start gap-4">
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #FE2DA3 0%, #8A2BE2 100%)",
          boxShadow: "0 0 24px rgba(254,45,163,0.55), inset 0 1px 0 rgba(255,255,255,0.3)",
        }}
      >
      ...
      <div className="min-w-0">
        ...
        <h2
          className="mt-1 text-3xl leading-tight tracking-wide text-[#F5F2EC]"
          style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
        >
          {t.title}
        </h2>
...
function ContextSummary({ plan, show }: { plan: PlanRow; show: string[] }) {
  ...
  if (show.includes("product_output") && plan.product_output) items.push({ label: "Your product", value: "Saved ✓" });
  ...
  return (
    <div className="rounded-md border border-[#8A2BE2]/40 bg-[#8A2BE2]/5 p-4">
      ...
      <ul className="space-y-1 text-sm text-[#F5F2EC]/80">
        {items.map((i) => (
          <li key={i.label}>
            <span className="font-bold text-[#F5F2EC]">{i.label}:</span>{" "}
            <span className="text-[#F5F2EC]/70">{i.value.length > 100 ? i.value.slice(0, 100) + "…" : i.value}</span>
          </li>
        ))}
      </ul>
    </div>
  ...
  return (
    ...
    >
      ...
      <label className="block">
        <span
          className="block text-lg sm:text-xl font-bold normal-case tracking-normal text-[#F5F2EC]"
          style={{ fontFamily: "var(--font-sub)" }}
        >
          {label}
        </span>
        {hint && (
          <span className="mt-2 block text-sm sm:text-[15px] font-normal normal-case tracking-normal text-[#F5F2EC]/65">
            {hint}
          </span>
        )}
        <div
          className={`relative mt-4 rounded-xl border bg-black/50 transition-all duration-200 ${
            active
    ...
    <div className="space-y-6">
      ...
      >
        <span
          ...
          style={{ background: "linear-gradient(90deg, #FE2DA3, #8A2BE2, #00F0D1)" }}
        />
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <h3
              className="text-2xl leading-tight tracking-wide text-slate-900 sm:text-3xl"
              style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
            >
              Find Your Lane
            </h3>
        ...
        <div className="prose-xcel space-y-6 text-[17px] leading-[1.75] text-slate-800">
          {sections.map((s, i) => (
            <section key={i}>
              {s.title && (
                <h4
                  className="mb-2 text-xl tracking-wide text-slate-900"
                  style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
                >
                  {s.title}
                </h4>
      ...
      {!streaming && (
        ...
        >
          <span
            ...
            style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase", letterSpacing: "0.04em" }}
          >
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between text-sm font-semibold text-muted-foreground">
        <span className="xcel-kicker text-[#00F0D1]">
          Side {safeIdx + 1} of {total}
        </span>
        <span className="hidden font-mono text-[10px] uppercase tracking-widest text-[#F5F2EC]/50 sm:inline">
          ◀ ▶ flip through the tracks
        </span>
      </div>
      <div
        key={safeIdx}
      ...
      >
        {current.title && (
          <h3
            className="mb-4 text-2xl leading-tight tracking-wide text-slate-900"
            style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
          >
            {current.title}
          </h3>
        )}
        <div className="prose-xcel text-[17px] leading-[1.75] text-slate-800">{renderMarkdown(current.body)}</div>
      </div>
      {total > 1 && (
            ...
            {sections.map((_, i) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all ${
                  i === safeIdx
                    ? "w-8 bg-[#FE2DA3] shadow-[0_0_8px_#FE2DA3]"
                    : "w-2.5 bg-white/15 hover:bg-[#00F0D1]/60"
                }`}
              />
            ))}
          </div>
  ...
  lines.forEach((raw, i) => {
    ...
    if (headerMatch && headerMatch[2] === "") {
      out.push(
        <h3
          key={i}
          className="mt-6 mb-3 text-xl font-bold tracking-wide text-slate-900"
          style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
        >
          {headerMatch[1]}
        </h3>,
...
function StorefrontWalkthrough() {
  ...
  const videos = [{ id: "wbWtUDfpacE", title: "Beacons.ai — Complete Beginner Walkthrough", platform: "Beacons" }];
  const steps = [
    {
      n: 1,
      t: "Create your account",
      d: "Go to beacons.ai. Sign up with the email you check every day — not a random one. Pick a username that's your name or your niche (e.g. @ClaireAtHome).",
    },
    {
      n: 2,
      t: "Add a real profile photo",
      d: "Take a fresh selfie in good light, smiling. No logo, no stock image. People buy from faces they trust.",
    },
    {
      n: 3,
      t: "Write your one-line bio",
      d: 'Format: "I help [who] do [what] without [the pain point]." Example: "I help busy moms get dinner on the table in 20 minutes — without takeout guilt."',
    },
    {
      n: 4,
      t: "Add your free thing first",
      d: "Before you sell anything, give a free PDF or checklist in exchange for an email. This is your list. Your list is the business.",
    },
    {
      n: 5,
      t: "Add your product block",
      d: "Upload your PDF/guide, set the price, write a 2-sentence pitch. Don't overthink the description — short beats clever.",
    },
    {
      n: 6,
      t: "Test it on your phone",
      d: "Open your link on your phone. Tap every button. Buy your own product with PayPal. If it works for you, it works for them.",
    },
  ];
  return (
          ...
          {videos.map((v) => (
            ...
            >
              ...
              <div className="flex items-center justify-between p-4">
                ...
                <span className="rounded-full border border-[#FE2DA3]/40 bg-[#FE2DA3]/10 px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-[#FE2DA3]">
                  ▶ YouTube
                </span>
              </div>
            </a>
        ...
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            <h3
              ...
              style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
            >
              The 6-step setup, no jargon
            </h3>
            <p className="mt-1 text-sm normal-case tracking-normal text-[#F5F2EC]/70">
              Knock these out in one sitting. About 45 minutes, start to finish.
            </p>
          </div>
        </div>
          ...
          {steps.map((s) => (
            <div key={s.n} className="rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p
                    className="text-base font-bold normal-case tracking-normal text-[#F5F2EC]"
                    style={{ fontFamily: "var(--font-sub)" }}
                  >
                    {s.t}
                  </p>
          ...
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              <h3
                ...
                style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
              >
                Your 30-day tour calendar
              </h3>
              <p className="mt-1 text-sm normal-case tracking-normal text-[#F5F2EC]/70">
                One square = one day. Click any day to expand it. Check 'em off as you knock 'em out.
              </p>
            </div>
          </div>
        ...
        <div className="rounded-2xl border-2 border-dashed border-[#00F0D1]/50 bg-[#00F0D1]/[0.05] p-6 shadow-[0_0_24px_rgba(0,240,209,0.12)]">
          <div className="mb-4 flex items-start gap-3">
            ...
            <div>
              <h3
                ...
                style={{ fontFamily: "Anton, sans-serif", textTransform: "uppercase" }}
              >
                {basics[platform].title}
              </h3>
              <p className="mt-1 text-sm normal-case tracking-normal text-[#F5F2EC]/70">
                No assumptions. Here's exactly which button to push.
              </p>
            </div>
          </div>
          <ol className="space-y-3">
            {basics[platform].steps.map((s, i) => (
              <li
                key={i}
                className="flex items-start gap-3 rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur"
              >
                <span
                ...
                <span className="text-base leading-relaxed normal-case tracking-normal text-[#F5F2EC]/85">
                  {splitBold(s)}
                </span>
              </li>
            ))}
  ...
  return (
    <div>
      <div className="grid grid-cols-5 gap-2 sm:grid-cols-7 sm:gap-3">
        {cells.map((n) => {
          ...
          return (
            ...
            >
              <span className="absolute left-2 top-1.5 font-mono text-[9px] font-bold uppercase tracking-widest opacity-70">
                Day
              </span>
      ...
      {open !== null && map.has(open) && (
        <div
          className="mt-4 rounded-xl border-2 border-[#FE2DA3]/50 p-5 shadow-[0_0_24px_rgba(254,45,163,0.20)] xcel-fade-up"
          style={{ background: "#F5F2EC" }}
        >
